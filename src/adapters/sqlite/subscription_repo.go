...
func (r *PromptRepo) Get(ctx context.Context, botID int) (string, error) {
	var prompt string
	err := r.db.QueryRowContext(ctx,
		`SELECT prompt FROM prompts WHERE bot_id = ?`, botID).Scan(&prompt)
	if err == sql.ErrNoRows {
		return "", nil
	}
	return prompt, err
...
func (r *PromptRepo) Save(ctx context.Context, botID int, prompt string) error {
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO prompts (bot_id, prompt) VALUES (?, ?) ON CONFLICT (bot_id) DO UPDATE SET prompt = ?`,
		botID, prompt, prompt)
	return err
}
...
func (r *OAuthRepo) SaveRefreshToken(ctx context.Context, userID int, provider, refreshToken string) error {
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO oauth_tokens (user_id, provider, refresh_token, updated_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP)
		 ON CONFLICT (user_id, provider) DO UPDATE SET refresh_token = ?, updated_at = CURRENT_TIMESTAMP`,
